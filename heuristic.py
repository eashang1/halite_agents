...
# Need to prioritize high density halite regions
# Need to prioritize attacking enemy bases
# Need to somehow defend our own bases
...
def agent(obs, config):
    ...
    for x in range(21):
        for y in range(21):
            if (board.cells[Point(x, y)].halite >= 50 and not any(
    ...
    for ship in me.ships:
        ...
        if (len(me.shipyards) == 0 and me.halite - cost >= 500) or \
                (
                        me.halite - cost >= 1200 and
                        board.step <= 320 and
                        dist_shipyard(ship, me.shipyards) > 6 and
                        ship.halite < 150):
            ...
            ship.next_action = None
        elif (ship.halite >= 500 or (ship.halite >= 75 and board.step >= 370)) and len(me.shipyards):
    ...
    for shipyard in me.shipyards:
        ...
        if ((len(me.ships) == 0 and me.halite - cost >= 500) or
            (me.halite - cost >= 500 and board.step <= 300)) and not nearby_ships(shipyard, me.ships):
